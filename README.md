...
```
                       ...
                       │  ├── Context API 상태 관리                   │
                       │  ├── React.lazy 코드 스플리팅                │
Binance WebSocket ──→  │  ├── 코인 셀렉터 (BTC/ETH/SOL)              │
(BTC/ETH/SOL ticks)    │  ├── 실시간 가격 (WebSocket)                │
                       │  ├── 변동성 예측 비교 차트                   │
CoinGecko API ────────→│  ├── 매매 시그널 + 적중률                   │
(가격, FNG, 거래량)     │  ├── 포트폴리오 시뮬레이터                  │
                       │  └── AI 마스코트 브리핑                      │
OpenAI API ───────────→├──────────────────────────────────────────────┤
(GPT-4o-mini)          │  FastAPI Backend                            │
                       │  ├── GARCH Models (5개) + Rate Limiting     │
                       │  ├── Monte Carlo (10K paths)                │
                       │  ├── Risk Score + Signal Engine             │
                       │  ├── Alembic 마이그레이션                    │
                       │  └── APScheduler (일일 데이터 수집)          │
                       ├──────────────────────────────────────────────┤
                       │  PostgreSQL / SQLite                        │
                       └─────────────────────────────────────────────┘
```
